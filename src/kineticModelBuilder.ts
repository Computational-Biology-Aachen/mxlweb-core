...
export class KineticModelBuilder extends ModelBuilderBase {
  ...
  /**
   * Reaction keys generated by `wireNNBlockOutputs` (ADR 0005 §2.1.3) — these
   * live in the same `this.reactions` map as hand-authored reactions (no
   * other marker distinguishes them), but must never be shown as ordinary
   * editable/deletable rows: a block's rate law is machine-generated and
   * changes only through fitting, never by hand. Every mxl-web surface that
   * lists `reactions` for editing must exclude this set.
   */
  nnBlockOwnedReactionNames(): Set<string> {
    const owned = new Set<string>();
    for (const [key, config] of this.nnBlocks) {
      config.targets.forEach((_, i) => owned.add(nnBlockReactionKey(key, i)));
    }
    return owned;
  }
